    ...
    pub fn parse_inputs(&self, graph: &ComputeGraph) -> Result<Vec<IBPTensor>, EvalError> {
        ...
        for var in &graph.invars {
            ...
            match tokens[i].as_str() {
                ...
                "point" => {
                    return Err(EvalError::Eval(format!(
                        "unexpected input marker 'point' (expected 'box')"
                    )));
                }
                other => {
                    return Err(EvalError::Eval(format!(
...
pub fn run_affine_bounds(args: AffineBoundsArgs) -> Result<(), EvalError> {
