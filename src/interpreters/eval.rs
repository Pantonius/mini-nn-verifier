use ndarray::{ArrayD, Axis, Ix0, Ix1, Ix2, IxDyn, Zip, arr0, linalg::Dot};
...
/// Plain 2-D matrix multiply.
fn matmul(a: &Tensor, b: &Tensor) -> Tensor {
    let a2 = a
        .clone()
        .into_dimensionality::<Ix2>()
        .expect("matmul lhs not 2-D");

    let b2 = b
        .clone()
        .into_dimensionality::<Ix2>()
        .expect("matmul rhs not 2-D");
...
fn expand_dims(a: &Tensor, axes: &[isize]) -> Tensor {
    // let result_ndim = a.ndim() + axes.len();
    // let mut norm: Vec<usize> = axes
    //     .iter()
    //     .map(|&ax| norm_axis_index(ax, result_ndim))
    //     .collect();
    // norm.sort_unstable();
    // let mut out = a.clone();
    // for pos in norm {
...
/// Reshape resolving a single `-1` (inferred) dimension.
...
fn pad(a: &Tensor, opt: &PaddingOptions) -> Tensor {
    // let nd = a.ndim();
    // let padded = |i: usize| opt.axes.iter().any(|&ax| norm_axis_index(ax, nd) == i);
    //
    // let out_shape: Vec<usize> = (0..nd)
    //     .map(|i| {
    //         let sz = a.shape()[i];
    //         if padded(i) && sz > 0 {
    //             opt.config.left + opt.config.right + sz + (sz - 1) * opt.config.interior
    //         } else if padded(i) {
    //             opt.config.left + opt.config.right
    //         } else {
    //             sz
    //         }
    //     })
    ...
    //     let dest: Vec<usize> = (0..nd)
    //         .map(|i| {
    //             if padded(i) {
    //                 opt.config.left + idx[i] * (opt.config.interior + 1)
    //             } else {
    //                 idx[i]
    //             }
    //         })
...
fn conv(input: &Tensor, kernel: &Tensor, stride: isize) -> Result<Tensor, EvalError> {
    ...
    //     )));
    // }
    // let (oh, ow) = ((h - kh) / s + 1, (w - kw) / s + 1);
    // let mut out = ArrayD::zeros(IxDyn(&[n, o, oh, ow]));
    // for ni in 0..n {
    //     for oi in 0..o {
    //         for i in 0..oh {
    //             for j in 0..ow {
    ...
    //                         for q in 0..kw {
    //                             acc +=
    //                                 input[[ni, ci, i * s + p, j * s + q]] * kernel[[oi, ci, p, q]];
    //                         }
    //                     }
    //                 }
    //                 out[[ni, oi, i, j]] = acc;
...
fn pool(a: &Tensor, opt: &PoolOptions, average: bool) -> Result<Tensor, EvalError> {
    // let nd = a.ndim();
    // if opt.window_size.len() != nd || opt.stride.len() != nd {
    //     return Err(EvalError::Eval(
    ...
    // let divisor = window_total as f64;
    //
    // let mut out = ArrayD::zeros(IxDyn(&out_shape));
    // for (oidx, slot) in out.indexed_iter_mut() {
    //     let mut acc = 0.0;
    //     for wl in 0..window_total {
    //         let mut rem = wl;
    //         let mut sidx = vec![0usize; nd];
    //         for i in (0..nd).rev() {
    //             let off = rem % opt.window_size[i];
    //             rem /= opt.window_size[i];
    //             sidx[i] = oidx[i] * opt.stride[i] + off;
    //         }
    //         acc += a[IxDyn(&sidx)];
    //     }
    //     *slot = if average { acc / divisor } else { acc };
    // }
    // Ok(out)
    todo!()
}
...
mod tests {
    ...
    fn dot_nd_md() {
        ...
        let b = ArrayD::from_elem(IxDyn(&[3, 4]), 1.0);
        let result = dot(&a, &b).unwrap();
    ...
    fn dot_1d_1d_mismatch() {
        let a = ArrayD::from_elem(IxDyn(&[3]), 1.0);
        let b = ArrayD::from_elem(IxDyn(&[4]), 1.0);
    ...
    fn dot_2d_2d_mismatch() {
        let a = ArrayD::from_elem(IxDyn(&[2, 3]), 1.0);
        let b = ArrayD::from_elem(IxDyn(&[4, 2]), 1.0);
        assert!(dot(&a, &b).is_err());
    }
    ...
    fn dot_nd_1d_mismatch() {
        let a = ArrayD::from_elem(IxDyn(&[2, 3, 4]), 1.0);
        let b = ArrayD::from_elem(IxDyn(&[5]), 1.0);
        assert!(dot(&a, &b).is_err());
    }
